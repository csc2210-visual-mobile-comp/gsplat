...
class Runner:
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        return render_colors, render_alphas, info, lora_splats
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            # forward
            renders, alphas, info, lora_splats = self.rasterize_splats(
            ...
            if cfg.opacity_reg > 0.0:
                loss += cfg.opacity_reg * torch.sigmoid(lora_splats["opacities"]).mean()
            if cfg.scale_reg > 0.0:
                loss += cfg.scale_reg * torch.exp(lora_splats["scales"]).mean()
            ...
            if world_rank == 0 and cfg.tb_every > 0 and step % cfg.tb_every == 0:
                ...
                self.writer.add_scalar("train/num_GS", len(lora_splats["means"]), step)
            ...
            if (
                ...
                means = lora_splats["means"]
                scales = lora_splats["scales"]
                quats = lora_splats["quats"]
                opacities = lora_splats["opacities"]
                export_splats(
                    means=means,
                    scales=scales,
                    quats=quats,
