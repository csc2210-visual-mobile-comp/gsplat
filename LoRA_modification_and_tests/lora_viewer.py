...
def main(local_rank: int, world_rank, world_size: int, args):
    ...
    if args.ckpt is None:
        ...
        for ckpt_path in args.ckpt:
            ...
            # 4. Check the ROOT checkpoint for our global lora_B matrix
            if "lora_B" in full_ckpt:
                lora_A = splats["lora_A"]  
                lora_B = full_ckpt["lora_B"]
                shN_computed = torch.matmul(lora_A, lora_B)
                shN_bands = lora_B.shape[1] // 3
                shN_computed = shN_computed.view(-1, shN_bands, 3)
                shN.append(shN_computed)
            else:
